...
Great example of the benefits to the code organization is the *CQRS* (Command Query Responsibility Segregation) architectural pattern.  
...
Using the ICommandHandler interface, we can implement concrete application operations.  
...
As you may have noticed in the Book Order example, the command handler have three responsibilities, of with two are general, and only one is specific for this particular use case.  
...
As we are dealing with lot of smaller classes, it is very helpful if our container have an API that allows us to use convention to register all of the generic Command Handler classes and their Aspect automatically.
Following example shows how to configure such convention with *Ninject* container (using *Ninject.Extensions.Conventions* package).
...
* [http://martinfowler.com/bliki/CommandQuerySeparation.html](http://martinfowler.com/bliki/CommandQuerySeparation.html)
